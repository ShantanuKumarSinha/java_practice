...
//    var dst = 2;
    ...
    firstDepthFirstSearch.findIfPathExist(graph, src, dst, visited);
    System.out.println(visited[dst]);
  }

  private void findIfPathExist(List<Integer>[] graph, int src, int dest, boolean[] visited) {
    if (src < graph.length) {
      visited[src] = true;
      for (int node : graph[src]) {
        if (node < graph.length && !visited[node]) findIfPathExist(graph, node, dest, visited);
      }
    }
  }
}
