...
public class PassingGame {
  public static void main(String[] args) {
    var result = getIdPossesingTheBall(10, 23, new int[] {86, 63, 60, 0, 47, 0, 99, 9, 0, 0});
    System.out.println(result);
  }

  public static int getIdPossesingTheBall(int numberOfPasses, int firstPerson, int[] idOfPasses) {
    Stack<Integer> stack = new Stack<>();
    if (numberOfPasses == 0) return firstPerson;
    stack.push(firstPerson);
    for (int i = 0; i < numberOfPasses; i++) {
      if (idOfPasses[i] == 0) {
        stack.pop();
      } else stack.push(idOfPasses[i]);
    }
    return stack.peek();
  }
}
